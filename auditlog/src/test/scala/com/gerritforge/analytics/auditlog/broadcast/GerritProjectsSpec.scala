...
class GerritProjectsSpec extends FlatSpec with Matchers {
  ...
  it should "extract a project from a gerrit query command, where the project is a well separated by a space" in {
    val project = "spdk/spdk.github.io"
  ...
  it should "extract a project from a gerrit query command, where the project is a well separated by curly brackets" in {
  ...
  it should "extract a project from a replication start command" in {
