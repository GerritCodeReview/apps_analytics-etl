...
object Main extends App with LazyLogging {
  ...
  Args.cliOptionParser.parse(args, AuditLogETLConfig()) match {
    case Some(config) =>
      ...
      val timeRange = TimeRange(config.since, config.until)
      ...
      val eventsDS = spark
        ...
        .filter(event => timeRange.isWithin(event.timeAtStart))
        .map(_.toJsonString)
        .toDS()
      ...
      if (!eventsDS.rdd.isEmpty()) {
        spark.sqlContext.read.json(eventsDS)
          .withColumn(timeBucketCol, date_trunc(format=config.eventsTimeAggregation.get, from_unixtime(timestampSecsCol)))
        ...
        logger.warn("Stop processing. Resulting dataSet is empty, nothing to do")
      }
...
object Args {
  val cliOptionParser: OptionParser[AuditLogETLConfig] =
    new scopt.OptionParser[AuditLogETLConfig]("spark-submit") {
      ...
      } text "time aggregation granularity: hour, month, year. Default 'hour'"
