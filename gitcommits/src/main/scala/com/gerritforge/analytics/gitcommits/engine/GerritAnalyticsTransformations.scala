...
object GerritAnalyticsTransformations {
  ...
  implicit class PimpedGerritProjectRDD(val rdd: RDD[GerritProjectWithBranch]) extends AnyVal {

    def enrichWithSource(
        projectToContributorsAnalyticsUrlFactory: GerritProjectWithBranch => Option[String]
  ...
  def getContributorStats(
                           projects: RDD[GerritProjectWithBranch],
                           projectToContributorsAnalyticsUrlFactory: GerritProjectWithBranch => Option[String],
                           gerritApiConnection: GerritConnectivity
