...
case class GerritEndpointConfig(
  ...
  private lazy val manifestProjectNames: Option[Set[String]] = manifest.map { mf =>
  ...
  val projectInManifest: GerritProject => Boolean = (project: GerritProject) => manifestProjectNames.fold(true)(_.contains(project.name.stripSuffix(".git")))
