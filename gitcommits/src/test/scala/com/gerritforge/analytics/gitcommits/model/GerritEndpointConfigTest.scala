...
class GerritEndpointConfigTest extends FlatSpec with Matchers with ManifestXML {
  ...
  it should "recognise projects contained or not in a repo manifest XML" in {
    val conf = GerritEndpointConfig(baseUrl = Some("testBaseUrl"), manifest = Option(manifestFile.getAbsolutePath))

    conf.projectInManifest(GerritProject("id1", "sel4_projects_libs")) shouldBe true
    conf.projectInManifest(GerritProject("id1", "non-existent-project")) shouldBe false
  }

  it should "not consider the '.git' extension of the project in the repo manifest XML" in {
    val conf = GerritEndpointConfig(baseUrl = Some("testBaseUrl"), manifest = Option(manifestFile.getAbsolutePath))

    conf.projectInManifest(GerritProject("id1", "sel4_projects_libs.git")) shouldBe true
    conf.projectInManifest(GerritProject("id1", "sel4runtime")) shouldBe true
  }
}
