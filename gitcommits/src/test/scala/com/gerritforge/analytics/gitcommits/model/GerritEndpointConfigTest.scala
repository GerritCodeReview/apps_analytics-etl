...
class GerritEndpointConfigTest extends FlatSpec with Matchers with ManifestXML {
  ...
  "contributorsUrl" should "" in {
    val conf = GerritEndpointConfig(baseUrl = Some("testBaseUrl"), prefix = None)
    conf.contributorsUrl(GerritProject("opensbi","opensbi",Some("refs/tags/v0.8"))) should be
  ...
  it should "return projects with the version contained in a repo manifest XML with" in {
    ...
    projectNamesFromManifest should contain only (
      GerritProject("musllibc","musllibc",Some("sel4")),
      GerritProject("nanopb","nanopb",Some("refs/tags/0.4.3")),
      GerritProject("opensbi","opensbi",Some("refs/tags/v0.8")),
      GerritProject("prefix%2Futil_libs","prefix/util_libs",Some("master")),
      GerritProject("sel4_projects_libs","sel4_projects_libs",Some("master")),
      GerritProject("sel4runtime","sel4runtime",Some("master")),
      GerritProject("sel4test","sel4test",Some("master")),
      GerritProject("seL4_libs","seL4_libs",Some("master")),
      GerritProject("seL4_tools","seL4_tools",Some("master"))
    )
  }
}
