...
class GerritAnalyticsTransformationsSpec
  ...
  "addManifestInfo" should "enrich the data with manifest-label name from the configuration" in {
    import spark.implicits._
    ...
    val df = inputDF.addManifestInfo("testManifestLabel", None)
  ...
  it should "enrich the data with manifest-label name and manifest branch from the configuration" in {
    import spark.implicits._
