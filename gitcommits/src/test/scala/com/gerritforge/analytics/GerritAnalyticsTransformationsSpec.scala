...
class GerritAnalyticsTransformationsSpec
  ...
  "transformCommitterInfo" should "transform a DataFrame with project and json to a workable DF with separated columns" in {
    ...
    df.schema.fields.map(_.name) should contain inOrder ("project", "project_version", "author", "email",
    "year", "month", "day", "hour",
  ...
  it should "transform an RDD to a DataFrame containing project name and version" in {
    ...
    df.schema.fields.map(_.name) should contain inOrder ("project", "project_version")
