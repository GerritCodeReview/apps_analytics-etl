...
class GerritAnalyticsTransformationsSpec
  ...
  "GerritProjects" should "parse JSON into a GerritProject objects" in {
    ...
    projects should contain only (GerritProjectWithBranch("All-Projects-id", "All-Projects-name"), GerritProjectWithBranch(
      "Test-id",
      "Test-name"
    ))
  }
  ...
  "enrichWithSource" should "enrich project RDD object with its source" in {
    ...
    val projectRdd = sc.parallelize(Seq(GerritProjectWithBranch("project-id", "project-name")))
  ...
  it should "enrich project RDD object with branch" in {
    ...
    val branch = "aBranch"
    val projectRdd = sc.parallelize(Seq(GerritProjectWithBranch("project-id", "project-name", Some(branch))))
  ...
  "addProductInfo" should "enrich the data with product name from the configuration" in {
    import spark.implicits._
    ...
    val df = inputDF.addProductInfo("testProduct", None)
  ...
  it should "enrich the data with product name and manifest branch from the configuration" in {
    import spark.implicits._
    ...
    val expectedDF = sc
      .parallelize(
        Seq(
          ("input_value1", productName, manifestBranch),
          ("input_value2", productName, manifestBranch)
        )
      )
      .toDF("input", "product_name", "manifest_branch")

    val df = inputDF.addProductInfo("testProduct", Some(manifestBranch))

    df.schema.fields.map(_.name) should contain allOf ("input", "product_name", "manifest_branch")
