...
class ElasticSearchPimpedWriterIT
  ...
  "Saving and reading from same ES alias" must "work while changing indices mapping" in {
    ...
    Await.result(dataIntoIndexOne.saveToEsWithAliasSwap(aliasName, documentName)._1, 2 seconds)
    ...
    Await.result(dataIntoIndexTwo.saveToEsWithAliasSwap(aliasName, documentName)._1, 2 seconds)
    // Reading from the alias
  ...
  "Saving data into ES" must "succeed even if alias creation fails" in {
    ...
    Await.result(aliasCreationReponse, 2 seconds) shouldEqual (None)
