...
trait ElasticSearchAliasOps {
  ...
  def getIndicesFromAlias(aliasName: String): Future[immutable.Iterable[Index]] = {
    ...
    esClient
      ...
      .map(_.result.mappings.map(_._1))

  }

  def replaceAliasOldIndicesWithNew(aliasName: String,
                                    newIndexName: String): Future[Response[AliasActionResponse]] = {
    val oldIndices: Future[immutable.Iterable[Index]] = getIndicesFromAlias(aliasName)

    import com.sksamuel.elastic4s.http.ElasticDsl._

    oldIndices.flatMap { indices =>
      val removeIdxActions: immutable.Iterable[RemoveAliasAction] = indices.map { idxName =>
        removeAlias(aliasName) on s"${idxName.name}"
      }
      val addIdxAction: AddAliasActionRequest = addAlias(aliasName) on newIndexName
      ...
      esClient.execute {
        aliases(
          removeIdxActions ++ List(addIdxAction)
        )
      }
    }
  }
