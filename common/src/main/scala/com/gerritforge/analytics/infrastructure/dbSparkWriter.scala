...
import com.gerritforge.analytics.support.ops.NameGenerator
...
class RelationalDatabasePimpedWriter[T](dataset: Dataset[T]) extends RelationalDatabaseViewOps
  ...
  def saveToDb(jdbcString: String, tableName: String, additionalProperties: Properties =  new Properties()): EnrichedAliasActionResponse = {
    val newTableNameWithTime = NameGenerator.timeBasedName(tableName, Instant.now())

    logger.info(
      s"Storing data into table $newTableNameWithTime and updating view $tableName to read from the new table")

    // Save data
    val futureResponse: Future[Boolean] = try {
      dataset.write
        .jdbc(jdbcString, newTableNameWithTime, additionalProperties)

      logger.info(
        s"Successfully stored the data into table $newTableNameWithTime. Will now update the view $tableName")
      ...
      updateViewToTheNewTable(newTableNameWithTime, tableName)

    } catch {
      case exception: Exception => Future.failed(exception)
    }

    EnrichedAliasActionResponse(futureResponse, newTableNameWithTime)
  }
