...
object GerritEventsTransformations extends LazyLogging {
  ...
  implicit class PimpedJsonRDD(val self: RDD[String]) extends AnyVal {
    def parseEvents(implicit eventParser: GerritJsonEventParser)
      : RDD[Either[NotParsableJsonEvent, GerritJsonEvent]] = {
  ...
  implicit class PimpedGerritJsonEventRDD[T <: GerritJsonEvent](val self: RDD[T]) extends AnyVal {
    ...
    def repositoryWithNewRevisionEvents: RDD[GerritRefHasNewRevisionEvent] = self.collect {
      case e: GerritRefHasNewRevisionEvent => e
    }
  }

  implicit class PimpedRepositoryModifiedGerritEventRDD(val self: RDD[GerritRefHasNewRevisionEvent])
      extends AnyVal {
    ...
    def userActivitySummaryPerProject(
        aggregationStrategy: AggregationStrategy,
        eventParser: GerritJsonEventParser): RDD[(String, UserActivitySummary)] = {
      self
        ...
        .flatMap {
          case ((project, _), changesPerUserAndTimeWindow) =>
            val dateParts: aggregationStrategy.DateTimeParts =
              aggregationStrategy.decomposeTimeOfAggregatedEvent(changesPerUserAndTimeWindow.head)
  ...
  implicit class PimpedUserActivitySummaryPerProjectRDD(
      val self: RDD[(String, UserActivitySummary)])
      extends AnyVal {
  ...
  def tryParseGerritTriggeredEvent(eventJson: String)(implicit eventParser: GerritJsonEventParser)
    : Either[NotParsableJsonEvent, GerritJsonEvent] = {
  ...
  private def addCommitSummary(summaryTemplate: UserActivitySummary,
                               changes: Iterable[ChangeMergedEvent]): UserActivitySummary = {
    ...
    summaryTemplate.copy(
      ...
      commits = changes
        .map(
          changeMerged =>
            CommitInfo(changeMerged.newRev,
                       changeMerged.eventCreatedOn * 1000,
                       changeMerged.patchSet.parents.size > 1))
        .toArray,
  ...
  def extractUserActivitySummary(changes: Iterable[ChangeMergedEvent],
    ...
    changes.headOption.fold(List.empty[UserActivitySummary]) { firstChange =>
      ...
      val (mergeCommits, nonMergeCommits) =
        changes.foldLeft(List.empty[ChangeMergedEvent], List.empty[ChangeMergedEvent]) {
          case ((mergeCommits, nonMergeCommits), currentCommit) =>
  ...
  def convertAsDataFrame(self: RDD[(String, UserActivitySummary)])(
      ...
      .map {
        case (project, summary) =>
          (project,
           ...
           summary.hour,
           summary.num_files,
           summary.num_distinct_files,
           summary.added_lines,
           summary.deleted_lines,
           Option(summary.num_commits),
           Option(summary.last_commit_date),
           Option(summary.is_merge),
           summary.commits,
           summary.branches)
      }
      .toDF(
  ...
  def getContributorStatsFromGerritEvents(
      events: RDD[GerritRefHasNewRevisionEvent],
