...
package com.gerritforge.analytics.common.ops
...
package AnalyticsTimeOps {
  ...
  object implicits {
    ...
    implicit class StringToTimeParsingOps(dateStr: String) {
      def parseStringToUTCSecondsEpoch(stringFormat: DateTimeFormatter): Long =
        LocalDateTime.parse(dateStr, stringFormat).convertToUTCEpoch
    }
