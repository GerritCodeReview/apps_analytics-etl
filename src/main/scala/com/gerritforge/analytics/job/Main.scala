...
trait Job { self: LazyLogging with FetchProjects =>
  ...
  def buildProjectStats()(implicit config: GerritEndpointConfig, spark: SparkSession): DataFrame = {
    ...
    require(config.baseUrl.isDefined, "Gerrit base url needs to be defined")
    val gerritApiService: GerritServiceApi = new GerritServiceApi(config.baseUrl.get, config.maybeUsername, config.maybeUsername)
